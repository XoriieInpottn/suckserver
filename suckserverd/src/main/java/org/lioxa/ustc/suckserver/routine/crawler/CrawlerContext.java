...
public class CrawlerContext implements Context {
     ...
     */
    public CrawlerRoutine createRoutineTree(String template) throws RoutineTreeException {
     ...
     * @throws RoutineTreeException
     */
    public CrawlerRoutine createRoutineTree(InputStream is) throws RoutineTreeException {
        TemplateReader reader = new TemplateReader();
        TemplateNode root;
        try {
            root = reader.read(is);
     ...
     * @throws RoutineTreeException
     */
    CrawlerRoutine createRoutineTreeFromDOM(TemplateNode root) throws RoutineTreeException {
        //
        ...
        List<Routine<CrawlerContext>> subRoutines = new ArrayList<>(children.size());
    ...
    CrawlerRoutine createRoutine(String name) throws RoutineTreeException {
        ...
        for (String word : words) {
            ...
            sb.append(word);
            sb.setCharAt(sb.length() - word.length(), Character.toUpperCase(word.charAt(0)));
        }
        name = sb.toString();
        //
        // Create instance.
        ...
        try {
            ...
            String err = String.format("Failed to create routine \"%s\".", name);
            throw new RoutineTreeException(err, e);
        }
        if (!(instance instanceof CrawlerRoutine)) {
            //
            // The class is not a subclass of CrawlerRoutine.
            String err = String.format("Failed to create routine \"%s\". Not a subclass of CrawlerRoutine", name);
