 ...
 */
@Order(4000)
public class Scan_bak1 extends CrawlerRoutine {
...
//				String sql = "update " + this.name + " set _isvisited = 1 where " + this.field + " = " + content + " and ";
				num ++;
			}
			a = this.after;
			list = scan(after);
	...
	public synchronized List<String> scan(long after) throws ExecutionException {
		...
		String sql = "SELECT " + this.field + " FROM " + this.name + " where _isvisited = 0 ORDER BY _timestamp ASC";
		String sql2 =  "SELECT MAX(_timestamp) FROM " + this.name;
		if (after > 0) {
			sql = "SELECT " + this.field + " FROM " + this.name + " where _timestamp > " + after + " and _isvisited = 0 ORDER BY _timestamp ASC";
		}
